...
GLuint VertexArrayIDLimbCube = 0;
static void createLimbCube()
{
	GLuint vertexbuffer;
	GLuint normalbuffer;
	GLuint uvbuffer;

	glGenVertexArrays(1, &VertexArrayIDLimbCube);
	glBindVertexArray(VertexArrayIDLimbCube);
	...
	// Two UV coordinatesfor each vertex.
	static const GLfloat g_uv_buffer_data_arm[] = {
	...
	glGenBuffers(1, &uvbuffer);
	glBindBuffer(GL_ARRAY_BUFFER, uvbuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(g_uv_buffer_data_arm), g_uv_buffer_data_arm, GL_STATIC_DRAW);
